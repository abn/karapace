...
async def test_happy_flow(default_config_path, mock_registry_client):
    serializer = SchemaRegistrySerializer(config_path=default_config_path)
    deserializer = SchemaRegistryDeserializer(config_path=default_config_path)
    serializer.registry_client = mock_registry_client
    deserializer.registry_client = mock_registry_client
    for o in serializer, deserializer:
        assert len(o.ids_to_schemas) == 0
        assert len(o.subjects_to_schemas) == 0
    for o in test_objects:
        assert o == await deserializer.deserialize("top", await serializer.serialize("top", o))
    for o in serializer, deserializer:
        assert len(o.ids_to_schemas) == 1
        assert 1 in o.ids_to_schemas
        assert len(o.subjects_to_schemas) == 1
        assert "top" in o.subjects_to_schemas
...
async def test_deserialization_fails(default_config_path, mock_registry_client):
    ...
    with pytest.raises(InvalidMessageHeader):
        await deserializer.deserialize("topic", invalid_header_payload)

    # for now we ignore the packed in schema id
    invalid_data_payload = struct.pack(">bII", START_BYTE, 1, 500)
    with pytest.raises(InvalidPayload):
        await deserializer.deserialize("topic", invalid_data_payload)
    ...
    with pytest.raises(InvalidPayload):
        await deserializer.deserialize("topic", enc_bytes)
